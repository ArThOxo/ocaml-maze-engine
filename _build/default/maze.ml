...
let usage () =
  ...
  print_endline "  ./maze.exe animate <file.laby>         : Solve with live animation";
...
(* Tente de charger un labyrinthe, le valide et l'anime en direct *)
let run_animate filename =
  try
    let g = Grid.from_file filename in
    if Grid.check g then begin
      if Grid.solve_animated g then
        print_endline "\nPath found!"
      else
        print_endline "\nNo solution found."
    end else
